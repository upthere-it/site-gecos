...
async function getCompany(): Promise<{ data: CompanyData; error: string | null }> {
  try {
    const res = await fetch(`${CONTENT_API_URL}/api/v1/company`, {
      cache: "no-store",
    });
    if (!res.ok) {
      return { data: EMPTY_COMPANY, error: `Backend ha risposto ${res.status}` };
    }
    ...
    return {
      data: EMPTY_COMPANY,
      error:
        err instanceof Error ? err.message : "Errore di comunicazione con il backend",
    };
  }
}
      ...
      {error && (
        <div className="max-w-3xl mx-auto mb-6">
          <p className="text-sm text-error border border-error/30 bg-error/5 px-4 py-3">
            Impossibile caricare i dati ({error}). Modifiche partiranno da un form vuoto.
          </p>
        </div>
      )}
